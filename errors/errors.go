...
func E(args ...interface{}) error {
	...
	for _, arg := range args {
		...
		case Path:
			e.Path = arg
		case string:
			// strings will be treated as a second error message.
			// Check if the Path was not converted
			if strings.ContainsRune(arg, os.PathSeparator) {
				// do not treat as path, errors message may, but should not, contain / or \
				_, file, line, _ := runtime.Caller(1)
				log.Printf("errors.E: message contains a path seperator: forgot to convert %s to errors.Path()? in %s:%d", arg, file, line)
			}
			e.Err = Str(arg)
		case Kind:
			e.Kind = arg
