...
// DO NOT REORDER
var testvfs = []vfs.File{
	{Relpath: "/", CTime: 1626984593612799116, MTime: 1626984593612799116, Mode: 0x800001ed, Inode: 0x2417c8, Size: 4096, Children: []vfs.File{
...
func fakeExploration(dirc chan<- *vfs.File, wg *sync.WaitGroup) {
	// do not reorder, this is real life insertion order. Easier to grasp than loop and checking for IsDir,....
	dirc <- &testvfs[0]
	dirc <- &testvfs[0].Children[3]
	dirc <- &testvfs[0].Children[3].Children[3]                         // hpi
	dirc <- &testvfs[0].Children[3].Children[4]                         // tum
	dirc <- &testvfs[0].Children[3].Children[3].Children[0]             // hpi/application
	dirc <- &testvfs[0].Children[3].Children[3].Children[0].Children[2] // hpi/application/wise202122
	dirc <- &testvfs[0].Children[3].Children[4].Children[0]             // tum/application
	dirc <- &testvfs[0].Children[3].Children[4].Children[0].Children[0] // tum/application/wise202122
	close(dirc)
	wg.Done()
}
...
func TestLoadAndStoreIndex(t *testing.T) {
	...
	difvfs := idx.DetailedEquals(reIdx)
	if len(difvfs) > 0 {
