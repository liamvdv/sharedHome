...
// inserted from afero/afer_test.go - modified for local changes
...
func testDir(fs Fs) string {
	name, err := TempDir(fs, "", "sharedHome")
	if err != nil {
...
func TestMultipleOpenFiles(t *testing.T) {
	...
	for i, fs := range Fss {
		...
		if err != nil {
			t.Error(fs.Name()+".Create failed: " + err.Error())
		}
...
func TestMemFsDataRace(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < n; i++ {
			...
			if err := fs.WriteFile(fname, []byte(""), 0777); err != nil {
				panic(err)
			}
...
func TestMemFsUnexpectedEOF(t *testing.T) {
	...
	if err := fs.WriteFile("file.txt", []byte("abc"), 0777); err != nil {
		t.Fatal(err)
	}
